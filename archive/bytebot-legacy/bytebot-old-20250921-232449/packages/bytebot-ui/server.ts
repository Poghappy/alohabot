...
// Backend URLs
const VINCENT_AGENT_BASE_URL = process.env.VINCENT_AGENT_BASE_URL;
const VINCENT_DESKTOP_VNC_URL = process.env.VINCENT_DESKTOP_VNC_URL;
...
app
  ...
  .then(() => {
    ...
    expressApp.use("/api/proxy/websockify", (req, res) => {
      ...
      // Rewrite path
      const targetUrl = new URL(VINCENT_DESKTOP_VNC_URL!);
      req.url =
        targetUrl.pathname +
